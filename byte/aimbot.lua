...
AimbotMainTab:AddDropdown("AutoLookPart", { Text = "Look Target", Values = { "Head (ban risk)", "Body", "Full Body", "Random" }, Default = 3 })
...
AimbotSettingsTab:AddSlider("PredictionVelocity", { Text = "Prediction Scale", Default = 0.2, Min = 0, Max = 15, Rounding = 5, Suffix = "" })
...
local function CreateDrawingESP(player)
    ...
    ESP_Cache[player] = {
        Tracer = Drawing.new("Line"), Box = Drawing.new("Square"), Name = Drawing.new("Text"), Health = Drawing.new("Text"), Distance = Drawing.new("Text"), Skeleton = {}
    }
...
local function CreateBackupESP(player)
    ...
    healthLabel.Position = UDim2.new(-0.4, 0, 0, 0); healthLabel.Size = UDim2.new(0.3, 0, 1, 0); healthLabel.BackgroundTransparency = 1; healthLabel.TextColor3 = Color3.fromRGB(160, 30, 255); healthLabel.TextStrokeTransparency = 0; healthLabel.Parent = infoList
    ...
    sBox.Adornee = char; sBox.Color3 = Color3.fromRGB(140, 0, 255); sBox.Parent = BackupGui
...
local function GetBestPart(player, mode)
    ...
    elseif mode == "Full Body" then
        ...
        if primary and IsVisible(player, primary) then
            return primary
        end
                ...
                if distance <= maxDistance then
                    ...
                    if useBackup then
                        ...
                        if d and onScreen then
                            ...
                            d.Box.Visible = Toggles.TwoDBox.Value; d.Box.Size = Vector2.new(width, height); d.Box.Position = Vector2.new(xPos, yPos); d.Box.Color = color; d.Box.Thickness = thickness; d.Box.Filled = false
                            d.Tracer.Visible = Toggles.Tracers.Value; d.Tracer.From = tracerFromPos; d.Tracer.To = Vector2.new(topPos.X, botPos.Y - (height/2)); d.Tracer.Color = color; d.Tracer.Thickness = thickness
                            ...
                            d.Health.Visible = Toggles.HealthNumber.Value; d.Health.Text = tostring(math.floor(hum.Health)); d.Health.Position = Vector2.new(xPos - 25, yPos + (height / 2) - 7); d.Health.Color = Color3.fromRGB(160, 30, 255)
